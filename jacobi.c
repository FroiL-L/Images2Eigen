...
{ 
	...
	// Set eigenvector matrix equal to identity matrix
	for (ip = 1; ip <= n; ip++)
	{
		for (iq = 1; iq <= n; iq++) 
			V[ip][iq] = 0.0;
	...
	for (ip = 1; ip <= n; ip++)
	{
		b[ip] = w[ip] = S[ip][ip]; // Set b and w equal to diagonal of matrix, respectively
		z[ip] = 0.0; // Set all of z equal to 0
	}
	...
	{
		printf("%d\n", i);
		sm = 0.0; // Sum of absolute values in diagonal of matrix
		for (ip = 1; ip <= n - 1; ip++)
		{
			for (iq = ip + 1; iq <= n; iq++) 
				sm += fabs(S[ip][iq]);
		...
		// Case: Diagonal of matrix was all 0s
		if (sm == 0.0)
		{ 
		...
		if (i < 4) 
			tresh = 0.2 * sm / (n * n); else tresh = 0.0;
		for (ip = 1; ip <= n - 1; ip++)
		{
			for (iq = ip + 1; iq <= n; iq++)
			{
